...
const PinjamTableContent = async ({
  query,
  show,
  id,
}: {
  ...
  const rows =
    data.length >= minimumRows
      ...
      : [
          ...data,
          ...Array.from({ length: minimumRows - data.length }, (_, index) => ({
  ...
  return (
    <>
      <div className="min-h-screen max-w-md w-full overflow-y-auto rounded-t-3xl border-[5px] border-red-800 mt-5 bg-[#fbf5f0] p-4">
        {show && <DetailPinjam id={id} type="process" />}
...
export default function PinjamTable({
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <PinjamTableContent query={query} show={show} id={id} />
    </Suspense>
  );
}
